package com.core.lion.config.cache;
...
public class LocalCache {
    ...
    public static void put(String key, String value, long expire) {
        log.debug("添加缓存，key={}, value={}, expire={}秒", key, value, expire);
        if (expire > 0) {
    ...
    private static void remove(String key) {
        ...
        log.debug("懒惰淘汰策略: 移除超时失效数据, cache={}", cache);
    }

    private static void removeAll() {
        log.debug("定期删除策略: 开始执行, store={}", store);
        for (String key : store.keySet()) {
            Cache cache = store.get(key);
            if (cache.getExpire() > 0 && cache.getExpire() < System.currentTimeMillis()) {
                store.remove(key);
                log.info("定期删除策略: 移除超时失效数据, key={}, value={}, time={}", key, cache.getValue(), cache.getExpire());
            }
        }
    }
